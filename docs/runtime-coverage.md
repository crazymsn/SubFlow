...
“继续”有两种用途：暂停时恢复当前任务；翻译失败或结果缺译时，在工作线程退出后重试原任务的翻译。重试使用原工作目录、识别结果、断句结果及成功的翻译缓存，保留原视频和字幕任务设置继续生成字幕与配音。可以修正令牌、更换翻译模型或润色设置后再继续；工作目录暂时被占用导致重试失败时，入口仍保留。翻译错误弹窗也提供「继续翻译」。
