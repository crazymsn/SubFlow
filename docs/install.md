...
从 [Releases](https://github.com/crazymsn/SubFlow/releases/latest) 下载 `SubFlow-Windows-1.2.6.zip`，整夹解压后运行 `SubFlow\SubFlow.exe`。
...
从 Releases 下载 `SubFlow-macOS-1.2.6.zip`，解压后把 `SubFlow.app` 拖到「应用程序」。该包由 GitHub Actions 用 `scripts/build-macos.sh` 从同一仓库源码构建（Apple Silicon）。若 Gatekeeper 拦截，按住 Control 点击后选择打开。
