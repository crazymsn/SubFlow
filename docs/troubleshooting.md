...
| YouTube 提示登录 / 不是机器人 | 游客被拦，或 `youtube-cookies.txt` 只有 VISITOR/PREF 没有 LOGIN_INFO | 从已登录浏览器重新导出 Netscape Cookie（必须含 LOGIN_INFO/SID）。也可放到 `%APPDATA%\SubFlow\Cookies`。Chrome 127+ 无法直接读 Cookie 库；可完全退出 Chrome 后再点下载 |
