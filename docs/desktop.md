...
翻译失败或未完成时，保留当前视频、输出和工作目录，修正令牌、网络或翻译模型后点击「继续」。弹窗也提供「继续翻译」。重试会读取修正后的 API 令牌、所选翻译模型和润色设置，再次失败后仍保留入口。缓存校验通过时复用已有识别，从翻译阶段恢复；更换视频或影响上游处理的设置可能需要重新识别。不要删除作业文件，也不要让多个任务写同一输出。

![翻译失败后的继续入口](images/translation-retry.png)

上图为当前真实对话框的模拟 HTTP 503 演示，不包含账号数据。关闭弹窗后也可在任务栏点击「继续」。
